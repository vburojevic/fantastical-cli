...
`eventkit` commands access the system Calendar database via EventKit. macOS will prompt for Calendar access on first use. The helper is compiled with `swiftc` (Xcode Command Line Tools) the first time you run an `eventkit` command. EventKit access requires macOS 14+ (latest full‑access APIs).
